# Home — Repository Guidelines

Home is a household management app for the Tonic ecosystem (Layer 2) — properties, items, maintenance, warranties, and more. It depends on Dock (Layer 1) for ecosystem integration.

```
Layer 0  Frappe Core
Layer 1  Dock          ← required dependency
Layer 2  Home          ← this app
```
...
After adding strings, update all CSV files in `home/translations/`.

## Dock Integration

Home registers with Dock's app switcher, search, and notifications via hook declarations in `hooks.py`. No direct import of Dock code — use the hook system only.
...
```bash
bench build --app home                     # 1. Build frontend (if changed)
